...
describe('browser registry', () => {
  ...
  it('shares one namespace and registers tickers by id', async () => {
    ...
    expect(browser.available.news).toBe(instance?.element)
    expect(browser.getTicker('news')).toBe(instance?.element)
    expect(browser.getTickers().news).toBe(instance?.element)
  })
  ...
  it('replaces the same id without nesting wrappers', async () => {
    ...
    expect(replacement?.element?.dataset.direction).toBe('right')
  })
